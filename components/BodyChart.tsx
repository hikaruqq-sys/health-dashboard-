...
export default function BodyChart({ data }: { data: BodyMetric[] }) {
  ...
  const hasMuscle = data.some((d) => d.muscleMass != null);

  return (
    <div className="rounded-2xl border p-5" style={{ background: 'var(--bg-card)', borderColor: 'var(--border)' }}>
      <h2 className="text-sm font-semibold uppercase tracking-wide mb-4" style={{ color: 'var(--text-sub)' }}>{hasMuscle ? '体脂肪率 / 筋肉量' : '体脂肪率'}</h2>
